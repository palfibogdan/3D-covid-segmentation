...
from monai.handlers import CheckpointSaver, MeanDice, StatsHandler, ValidationHandler, TensorBoardStatsHandler, \
...
def get_xforms(mode = "train", keys = ("image", "label")):
	...
	xforms = [
		...
		Spacingd(keys, pixdim = (0.774776938, 0.774776938, 4.671889345), mode = ("bilinear", "nearest")[: len(keys)]),
	...
	if mode == "train":
		xforms.extend(
			[
				...
				RandCropByPosNegLabeld(keys, label_key = keys[1], spatial_size = (256, 256, 16), num_samples = 4),
...
class DiceCELoss(nn.Module):
	...
	def __init__(self):
		super().__init__()
		print('start init_loss function: DICE - CE ')
...
def get_dice_score(engine):
	"""Returns the dice score for each epoch. Used for early stopping"""
...
def train(data_folder = ".", model_folder = "."):
	...
	max_epochs, lr, momentum = 1000, 0.001, 0.95
	logging.info(f"epochs {max_epochs}, lr {lr}, momentum {momentum}, dropout {net.dropout}")

	opt = torch.optim.Adam(net.parameters(), lr = lr)
	logging.info(f"Optimizer: {opt}")

	# create evaluator (to be used to measure model quality during training
	val_post_transform = monai.transforms.Compose(
		[
			Activationsd(keys = "pred", softmax = True),  # Before: SIGMOID
	...
	evaluator = monai.engines.SupervisedEvaluator(
		...
		key_val_metric = {
			...
			"val_mean_diceloss": 1 - MeanDice(include_background = False,
			                                  output_transform = lambda x: (x["pred"], x["label"])),
		},
		val_handlers = val_handlers,
		amp = amp,
	)
	print('evaluator calculated:')
	...
	trainer = monai.engines.SupervisedTrainer(
		...
		key_train_metric = {"train_mean_dice": MeanDice(include_background = False, output_transform = lambda x: (
			x["pred"], x["label"]))},
	...
	lr_scheduler = ReduceLROnPlateau(opt, mode = 'min', factor = 0.1, patience = 10, threshold = 0.01, threshold_mode = 'rel')

	@evaluator.on(Events.COMPLETED)
	def reduct_step(engine):
		# engine is evaluator
		# engine.metrics is a dict with metrics, e.g. {"loss": val_loss_value, "acc": val_acc_value}
		lr_scheduler.step(engine.state.metrics['val_mean_diceloss'])
		logging.info(f"LR SCHEDULER. Current LR: {opt.param_groups[0]['lr']}")
...
def infer(data_folder = ".", model_folder = "runs", prediction_folder = "output"):
	...
	files = glob.glob(os.path.join(prediction_folder, "study*", "*.nii.gz"))
	for f in files:
		new_name = os.path.basename(f)
		new_name = new_name[len("study_0"):]
		new_name = new_name[: -len("_seg.nii.gz")] + ".nii.gz"
...
if __name__ == "__main__":
	"""
	Usage:
		python UNET.py train --data_folder x --model_folder y --pred_folder z" # run the training pipeline
		python UNET.py infer --data_folder x --model_folder y --pred_folder z" # run the inference pipeline
	"""
	...
	parser.add_argument("--pred_folder", default = "",
	                    type = str, help = "predicted data folder")
	...
	if args.mode == "train":
		...
		data_folder = args.data_folder
		infer(data_folder = data_folder, model_folder = args.model_folder, prediction_folder = args.pred_folder)
	else:
